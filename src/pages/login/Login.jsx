    ...
    if(!token){
      ...
      .then(({data})=>{
        if(data.success){
          ...
          Cookies.set('username', data.username, {expires: new Date(data.expires), secure: true, path: '/'})
          Cookies.set('user_id', data.user_id, {expires: new Date(data.expires), secure: true, path: '/'})
          navigate('/')
        }else{
    ...
    <div className='min-h-screen w-full bg-indigo-200 bg-cover bg-center flex justify-center items-center text-white font-medium'>
        <div dir='rtl' className=' mx-auto p-8 bg-gray-800 rounded-md shadow-md w-[350px]'>
          ...
          {newUser&& <>
            ...
            {newUserPopup && <>
            <div onClick={()=>setNewUserPopup(false)} className='fixed left-0 top-0 w-screen h-screen bg-layout backdrop-blur-sm animate-alert duration-100 z-[50]'></div>
