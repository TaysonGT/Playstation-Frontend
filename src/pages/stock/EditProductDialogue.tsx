...
const EditProductDialogue: React.FC<Props> = ({onEdit, product, onCancel}) => {
  ...
  return (
    <div dir={currentDirection} className='pt-8 px-16 pb-10 bg-white border-2 border-white rounded-lg fixed top-1/2 left-1/2 -translate-1/2 z-[120]'>
      <h1 className='text-3xl font-bold text-black text-center'>{t('stock.editProduct')}</h1>
        <form action="" className='flex flex-col mt-6'>
            <label className='mt-2'>{t('stock.productName')}</label>
            ...
            <label className=' mt-4'>{t('tables.quantity')}</label>
            ...
            <label className=' mt-4'>{t('tables.price')}</label>
            ...
            <div className='flex justify-between'>
                <button type='button' onClick={()=>onCancel} className='bg-white border-2 border-black hover:bg-black hover:text-white duration-200 rounded p-3 px-6 mt-10 text-xl font-bold'>{t('modals.cancel')}</button>
                <button type="submit" onClick={(e)=> {
                    e.preventDefault()
                    onEdit(product.id, form)
                }} className='text-white bg-indigo-500 hover:bg-indigo-300 duration-200 rounded  p-3 px-6 mt-10 text-xl font-bold'>{t('modals.save')}</button>
            </div>
        </form>
    </div>
  )
