...
const OrderPopup = ({setMessage, setSuccess, setShowPopup}) => {
  ...
  const inputHandler = (e)=>{
    ...
    if(inputs.length>0){
      inputs?.map((item)=> 
      item.value>0&&orders.push({product_id : item.id, quantity: parseInt(item.value)})
      )
    }else{
      ...
      <form action="" className='mt-4'>
        ...
        {products?.map((product, i)=>
          <tr key={i} className={'border-b text-md font-medium border-[#ddd] ' + (i%2===0? 'bg-[#f3f3f3] ' : 'bg-[#e3e3e3] ')+ (i===products.length-1&&' border-b-[4px] border-[#009829]')}>
            ...
            <td className='px-[15px] py-[12px] text-white'><input type="number" id={product.id} onInput={(e)=>inputHandler(e)} className="w-[50px] text-center p-1 input border-y-2 border-[#009829] bg-slate-700" defaultValue={0} /></td>
