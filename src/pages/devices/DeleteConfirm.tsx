...
const DeleteConfirm = ({deleteId, onAction, hide}:{deleteId:string|null, onAction: ()=>void, hide: ()=>void}) => {
    const deleteHandler = ()=>{
        axios.delete(`/devices/${deleteId}`, {withCredentials: true})
        .then(({data})=>{
            if(data.message){
    ...
    <div className='fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] bg-white shadow-large flex-col flex z-[102] select-none px-6 py-4 gap-6'>
        <h1 className='text-md font-bold'>هل تريد حذف هذا الجهاز؟</h1>
