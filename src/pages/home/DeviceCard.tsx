...
const DeviceCard = ({device}:{device: IDevice}) => {
  ...
  return (
    ...
    <div className='bg-[#ffffff] w-50 border-2 border-[#e0e0e0] flex flex-col items-center rounded p-5 text-[#333] shadow-lg duration-[.3s]'>
        ...
        <SessionDetails {...{show: showDetails&&!!device.session, device, clock, hide: ()=>{
            searchParams.delete('session')
            setSearchParams(searchParams)
            setShowDetails(false)
        }}} />
        ...
        <form dir={currentDirection} className='w-full flex flex-grow flex-col gap-1 text-xs font-bold mt-6'>
            ...
            <div className='flex justify-between gap-2 items-center mt-1'>
                <label className="font-bold text-xs flex-1 text-nowrap">{t('devices.timeType')}:</label>
                <select onInput={(e)=> timeTypeHandler(e.currentTarget.value)} className={classnames.input+' flex-1'}>
                    <option value="open">{t('devices.open')}</option>
                    <option value="time">{t('devices.time')}</option>
                </select>
            </div>
            <div className='mt-2 flex flex-col items-center'>
                <label className='font-bold text-sm'>{t('devices.time')}</label>
                <div className='flex w-full gap-6 px-4 mt-1 mb-2'>
            ...
            <button onClick={startDeviceHandler} id={device.id} className=' bg-[#37474f] text-white p-2 text-xs rounded-sm hover:bg-[#4f6874]  duration-150 active:shadow-hardInner mt-auto '>{t('devices.start')}</button>
        </form> :
            ...
            <div className={'w-[90%] p-2 text-center text-2xl font-bold bg-[#212121] text-white bg-center bg-cover border-emerald-50 border rounded  ' + (device.session?.time_type === "open"? 'text-red-500' :  'text-green-500')}>
                {clock? 
                    ...
                    <div className='w-full flex justify-center'>
                        <Loader size={30} color='white' thickness={5}/>
                    </div>
                }
            </div>
            <div className='flex justify-between gap-4 p-2 '>
                <p className={'text-xs font-medium p-3 py-2 rounded-sm w-full text-white ' + (device.session?.time_type==="open" ?  'bg-red-700': 'bg-teal-600')} >{device.session?.time_type.toUpperCase()}</p>
                <p className='text-xs font-medium p-3 py-2 bg-slate-700 rounded-sm w-full text-white'>{device.session?.play_type.toUpperCase()}</p>
            </div>
            <button onClick={()=> {
                setShowDetails(true)
                searchParams.set('session', device.id)
                setSearchParams(searchParams)
            }} id={device.id} className=' bg-cyan-500 p-2 text-xs rounded-sm text-white hover:bg-cyan-400 duration-200 active:shadow-hardInner mt-auto w-full text-center '>{t('devices.details')}</button>
        </div>
        }
