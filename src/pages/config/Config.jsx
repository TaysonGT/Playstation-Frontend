...
const Config = () => {
  ...
  useEffect(()=>{
    ...
    axios.get('/finances/users', {withCredentials: true})
    .then(({data})=>{
  ...
  return (
    <div dir='rtl' className="flex flex-col h-screen overflow-hidden pb-10 bg-[#0d47a1] pt-32 lg:px-36 px-10 font-alexandria">
      {typePopup&& <>
        ...
        <div onClick={(e)=>{e.preventDefault(); setTypePopup(false)}} className='fixed left-0 top-0 w-screen h-screen bg-layout backdrop-blur-sm animate-alert duration-150 z-[99]'></div>
      </>}
      {addPopup&& <>
